  ...
  'income-tax-law-1999': [
    {
      ...
      status: 'published',
      text: [
        'نموذج إقرار ضريبة الدخل السنوي',
        'قانون ضرائب الدخل رقم (31) لسنة 1999م — لائحة التنفيذ',
        '',
        'بيانات المكلف',
        '1. الاسم/الاسم التجاري: ____________________________',
        '2. رقم الهوية/السجل التجاري: ____________________________',
        '3. العنوان: ____________________________',
        '4. النشاط: ____________________________',
        '5. السنة المالية: 14__ / 14__ هـ',
        '',
        'حساب الدخل الخاضع للضريبة',
        '6. إجمالي الإيرادات: ________ ريال',
        '7. إجمالي المصروفات المخصومة: ________ ريال',
        '8. صافي الربح: ________ ريال',
        '9. الدخل الخاضع للضريبة: ________ ريال',
        '',
        'شرائح الضريبة',
        '10. الشريحة المعنية: ______',
        '11. النسبة المئوية: ______%',
        '12. الضريبة المستحقة: ________ ريال',
        '',
        'إقرار وتوقيع',
        'أقر بأن البيانات الواردة في هذا النموذج صحيحة ومطابقة للواقع.',
        '',
        'التاريخ: ____ / ____ / ____ هـ',
        '',
        'توقيع المكلف: ____________________________',
        '',
        'توقيع الممثل القانوني (للأشخاص الاعتبارية): ____________________________',
        '',
        'ملاحظة: يُقدّم هذا الإقرار خلال المدة المحددة قانونًا. تأخر التقديم يعرّض المكلف لغرامة.',
        'هذا النموذج بيانات تجريبية لأغراض العرض على منصة التشريعات اليمنية.',
      ].join('\n'),
    },
    {
  ...
  'constitution-2001': [
    {
      ...
      status: 'published',
      text: [
        'خريطة تقسيمات الجمهورية اليمنية',
        'وفقًا لدستور الجمهورية اليمنية الصادر 2001م',
        '',
        'الأقاليم والإحادات',
        '',
        '1. العاصمة: صنعاء — المركز الإداري للجمهورية.',
        '2. المحافظات (22 محافظة):',
        '   أ. صنعاء — صنعاء القديمة',
        '   ب. عدن — عاصمة الدولة الاقتصادية والمؤقتة',
        '   ج. الحديدة — أكبر الموانئ البحرية',
        '   د. تعز — المركز الثقافي والتجاري',
        '   هـ. إب — المحافظة الزراعية الخضراء',
        '   و. ذمار — المركز الجامعي',
        '   ز. حضرموت — أكبر المحافظات مساحة',
        '   ح. حجة — الشمال الجبلي',
        '   ط. صعدة — الحدود الشمالية',
        '   ي. عمران — الشمال المرتفع',
        '   ك. المحويت — المرتفعات الغربية',
        '   ل. البيضاء — الهضبة الوسطى',
        '   م. ريمة — المرتفعات الغربية',
        '   ن. الضالع — الجنوب الجبلي',
        '   س. لحج — الجنوب الساحلي',
        '   ع. أبين — دلتا وادي تبن',
        '   ف. شبوة — الهضبة الجنوبية',
        '   ص. مأرب — الوسط الشرقي',
        '   ق. المهرة — أقصى الشرق',
        '   ر. سقطرى — الأرخبيل الجزري',
        '',
        'المساحة الإجمالية: حوالي 555,000 كيلومتر مربع',
        'السكان (تقديري): أكثر من 30 مليون نسمة',
        'الحدود: السعودية (شمال)، عمان (شرق)، البحر الأحمر (غرب)،',
        'خليج عدن وباب المندب (جنوب)',
        '',
        'ملاحظة: هذه وثيقة وصفية للمرفق الخريطي. في بيئة الإنتاج،',
        'يُخدم الملف الأصلي (PNG/PDF) من مخزن الملفات (FileAsset).',
        'هذه البيانات تجريبية لأغراض العرض على منصة التشريعات اليمنية.',
      ].join('\n'),
    },
  ],
}
...
async function main() {
  ...
  for (const [slug, atts] of Object.entries(ATTACHMENTS)) {
    ...
    for (let i = 0; i < atts.length; i++) {
      ...
      // If the attachment exists but the seeded payload carries content
      // (text/table) that the DB row lacks, backfill it so re-seeding
      // upgrades older rows instead of being silently skipped.
      if (existing) {
        const needsText =
          att.text && !existing.textContent && att.contentType !== 'table'
        const needsTable =
          att.tableContent && !existing.tableContent && att.contentType === 'table'
        if (needsText || needsTable) {
          await db.attachment.update({
            where: { id: existing.id },
            data: {
              textContent: needsText ? att.text : existing.textContent,
              tableContent: needsTable ? att.tableContent : existing.tableContent,
            },
          })
        }
        continue
